...
CORTEX_SEARCH_SERVICES = "HR_SEARCH_SERVICE"
SEMANTIC_MODEL = '@"CORTEX_SEARCH_TUTORIAL_DB"."DIL_HR"."HR"/hr_v2.yaml'
...
if "service_metadata" not in st.session_state:
    st.session_state.service_metadata = [{"name": "HR_SEARCH_SERVICE", "search_column": ""}]
if "selected_cortex_search_service" not in st.session_state:
    st.session_state.selected_cortex_search_service = "HR_SEARCH_SERVICE"
...
def init_service_metadata():
    st.session_state.service_metadata = [{"name": "HR_SEARCH_SERVICE", "search_column": ""}]  # Default
    st.session_state.selected_cortex_search_service = "HR_SEARCH_SERVICE"  # Simple name
    if 'snowpark_session' not in st.session_state or st.session_state.snowpark_session is None:
        st.error("Snowpark session not initialized. Please authenticate first.")
        return
    try:
        session = st.session_state.snowpark_session
        svc_search_col = session.sql("DESC CORTEX SEARCH SERVICE HR_SEARCH_SERVICE;").collect()[0]["search_column"]
        st.session_state.service_metadata = [{"name": "HR_SEARCH_SERVICE", "search_column": svc_search_col}]
        #st.write(f"Service Metadata Updated: {st.session_state.service_metadata}")  # Debug log
    except Exception as e:
        st.error(f"❌ Failed to verify HR_SEARCH_SERVICE: {str(e)}. Using default configuration.")
...
# Query the Cortex search service to retrieve relevant hr data context for a given query.
...
if not st.session_state.authenticated:
    ...
    if st.button("Login"):
        try:
            conn = snowflake.connector.connect(
                ...
                host=HOST,
                port=443,
                warehouse="DART_DEV_WH",
                role="DEV_CORTEX_AI_USER",
                database=DATABASE,
                schema=SCHEMA,
            )
    ...
    # Ensure session is available for subsequent runs
    session = st.session_state.snowpark_session
    init_service_metadata()  # Call again to ensure metadata is initialized after rerun
    ...
    def snowflake_api_call(query: str, is_structured: bool = False):
        #st.write(f"Using Cortex Search Service: {st.session_state.selected_cortex_search_service}")  # Debug log
        payload = {
    ...
    def suggest_sample_questions(query: str) -> List[str]:
        try:
            prompt = (
                f"The user asked: '{query}'. This question may be ambiguous or unclear in the context of a business-facing hr analytics assistant. "
                f"Generate 3-5 clear, concise sample questions related to headcount, payroll, net pay, or hr metrics. "
                f"The questions should be easy for a business user to understand and answerable using hr data such as employee headcount, Total net pay, or offers accepted. "
                f"Format as a numbered list. Example format:\n1. Total count of employee headcount?\n2. How many applications interviewed?"
            )
    ...
    with st.container():
        st.markdown(
            ...
            <div class="fixed-header">
                <h1 style='color: #29B5E8; margin-bottom: 5px;'>   Cortex AI-HR Assistant by DiLytics</h1>
                <p style='font-size: 16px; color: #333;'><strong>Welcome to Cortex AI. I am here to help with Dilytics HR Insights Solutions</strong></p>
            </div>
            """,
            unsafe_allow_html=True
        )
            ...
            with st.spinner("Generating Response..."):
                ...
                if is_greeting or is_suggestion:
                    ...
                    if greeting not in ["hi", "hello", "hey", "greet"]:
                        greeting = "Hello"
                    response_content = f"{greeting}! I'm here to help with your hr analytics questions. Here are some questions you can ask me:\n\n"
                    selected_questions = sample_questions[:5]
                    for i, q in enumerate(selected_questions, 1):
                        response_content += f"{i}. {q}\n"
                    response_content += "\nFeel free to ask any of these or come up with your own related to hr analytics!"
